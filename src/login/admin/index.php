...
if (!in_array($_FILES['zaloha_zip']['type'], $allowed_zip)) {
$obnova_error = 'Nahraný soubor není ZIP archiv.';
} else {
$zip = new ZipArchive();
if ($zip->open($_FILES['zaloha_zip']['tmp_name']) === true) {
$ocekavane = ['data_novinky.json', 'data_tabulka.json'];
$nalezene = [];
...
if (!$obnova_error) $obnova_success = true;
}
$zip->close();
} else {
                ...
                <div>
                    ...
                    <form method="POST" enctype="multipart/form-data">
                        <div class="flex gap-2 items-center">
                            <input type="file" name="zaloha_zip" accept=".zip"
                                   id="zaloha-input"
                                   class="bg-[#2d2d2d] text-[#dfdfdf] px-4 py-2 rounded-lg">
                            <button type="button" onclick="document.getElementById('zaloha-input').value = ''"
                            ...
                            <button type="submit" name="obnova" onclick="return confirm('Opravdu chcete obnovit zálohu? Aktuální data budou přepsána!')"
                                    class="bg-blue-500 hover:bg-blue-600 px-4 py-2 rounded-lg transition-colors text-[#dfdfdf] font-semibold cursor-pointer whitespace-nowrap">
                                ⬆️ Obnovit zálohu
                            </button>
                        </div>
                    </form>
